...
- Read `README.md` for the project overview and supported installers, then review `.sangoi/docs/architecture-overview.md`, `.sangoi/docs/backend.md`, `.sangoi/docs/extensions-and-integrations.md`, `.sangoi/docs/frontend.md`, `.sangoi/docs/refactor-roadmap.md`, and `.sangoi/docs/testing-and-tooling.md`.
- Inspect the top entry of any handoff or session logs under `.sangoi/docs/` (create one if absent) before making changes.
...
- Document behaviour changes by updating the relevant guides in `.sangoi/docs/` as well as user-facing files (e.g., `README.md`, `NEWS.md`) when public experience shifts.
...
- GPU profiling helpers: enable `--backend-log-memory` and reuse the profiling hooks documented in `.sangoi/docs/testing-and-tooling.md`; prefer deterministic benchmarks from `scripts/` instead of ad-hoc timers.
...
- When modifying performance-sensitive code, capture before/after VRAM or latency metrics using the profiling hooks described in `.sangoi/docs/testing-and-tooling.md`.
...
- If new tasks arise, append them to the relevant roadmap or checklist under `.sangoi/docs/` and mention them in the handoff.
