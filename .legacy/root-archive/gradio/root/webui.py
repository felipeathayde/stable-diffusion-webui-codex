...
def webui_worker():
    ...
    while 1:
        ...
        current_port = cmd_opts.port

        def _find_fallback_port(start=20000, end=20999):
        ...
        while True:
            try:
                ...
                needs_fallback = (
                    ...
                    and current_port in (None, 0)
                    and "7860-7959" in message
                )
                if not needs_fallback:
                    raise

                fallback_port = _find_fallback_port()
                if fallback_port is None:
                    print("[PORT GUARD] Fallback range 20000-20999 exhausted; aborting launch.")
                    raise

                print("[PORT GUARD] ⚠️  Windows locked ports 7860-7959. Retrying on fallback port", fallback_port)
                _print_port_guard_banner(fallback_port)
                current_port = fallback_port
                fallback_attempted = True
                continue

        if fallback_attempted and (cmd_opts.port in (None, 0)):
            cmd_opts.port = current_port
...
if __name__ == "__main__":
    ...
    main_thread.loop()
def _print_port_guard_banner(port: int):
    message = (
        "#\n"
        "#  PORT GUARD ACTIVATED"\
        f" (rerouted to port {port})\n"
        "#  Windows reported all ports in 7860-7959 as busy.\n"
        "#  You can set --port / GRADIO_SERVER_PORT to pin a value.\n"
        "#\n"
    )
    if _HAS_COLOR:
        banner = (
            f"{Fore.YELLOW}{Style.BRIGHT}##############################################\n"
            f"##           PORT GUARD ACTIVATED          ##\n"
            f"##   Default range locked; using {port:<8}      ##\n"
            f"##############################################{Style.RESET_ALL}\n"
        )
