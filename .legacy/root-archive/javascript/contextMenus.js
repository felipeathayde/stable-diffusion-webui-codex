...
(function initDefaultMenus() {
    try {
        // Prefer modular context-menu installer if available
        // @ts-ignore
        if (window?.sdw?.helpers?.contextMenu?.install) {
            // Delegate default menus + listeners to the modular helper and skip legacy init
            // @ts-ignore
            window.sdw.helpers.contextMenu.install();
            return;
        }
    } catch {}
...
try {
    // Prefer modular hook when present
    // @ts-ignore
    if (window?.sdw?.helpers?.contextMenu?.install) {
        // @ts-ignore
        onAfterUiUpdate?.(window.sdw.helpers.contextMenu.install);
    } else {
