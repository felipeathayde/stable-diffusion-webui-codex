...
function isHTMLElement(node) { return !!(node && node instanceof HTMLElement); }
...
function setupExtraNetworksForTab(tabname) {
    ...
    tabChildren.forEach((child) => {
        ...
        const applySort = (force = false) => {
            const cards = /** @type {HTMLElement[]} */ (Array.from(root.querySelectorAll(`#${tabnameFull} div.card`)).filter((node) => node instanceof HTMLElement));
...
function extraNetworksTreeProcessDirectoryClick(
    ...
    const ul = guardElement(btn.nextElementSibling, /** @type {(n: Element) => n is HTMLElement} */ (n => n instanceof HTMLElement));
    if (!ul) return;
