...
function updateTooltip(element) {
    ...
    const translateKey = (key) => {
        ...
        const lookup = titles[key];
        if (!lookup) return undefined;
        const translated = localization[lookup];
        return typeof translated === 'string' ? translated : lookup;
    };
    ...
    if (!tooltip) {
        const label = (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement)
            ? element.placeholder || ''
            : element.innerText || '';
        tooltip = translateKey(label) || '';
    }

    if (!tooltip && element.dataset) {
        tooltip = translateKey(element.dataset.value) || '';
    }

    if (!tooltip) {
        for (const cls of Array.from(element.classList)) {
            const translated = translateKey(cls);
            if (translated) {
                tooltip = translated;
                break;
            }
...
onUiUpdate((/** @type {MutationRecord[] | MutationRecord | null | undefined} */ mutationRecords) => {
    ...
    if (tooltipCheckNodes.size) {
        if (tooltipCheckTimer !== null) window.clearTimeout(tooltipCheckTimer);
