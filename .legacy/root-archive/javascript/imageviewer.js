...
onAfterUiUpdate(() => {
    try {
        // @ts-ignore
        if (window?.sdw?.helpers?.lightbox?.afterUpdate) {
            // @ts-ignore
            return void window.sdw.helpers.lightbox.afterUpdate();
        }
    } catch {}
...
try {
    const lightboxHelper = window?.sdw?.helpers?.lightbox;
    if (lightboxHelper && typeof lightboxHelper.install === 'function') {
        document.addEventListener('DOMContentLoaded', () => {
            try { lightboxHelper.install(); } catch { buildModal(); }
        });
    } else {
