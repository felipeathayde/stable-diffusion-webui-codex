...
// @ts-check
/*
 Delegates to modular no-build helper if available to avoid duplicate listeners.
*/
...
if (typeof window !== 'undefined' && window?.sdw?.helpers?.lightboxGamepad?.install) {
    try { window.sdw.helpers.lightboxGamepad.install(); } catch {}
    // Skip legacy bindings to prevent double handlers
} else {
