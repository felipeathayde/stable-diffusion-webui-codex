...
onOptionsChanged(function() {
    ...
    const rawBefore = toRecord(opts._comments_before);
    const rawAfter = toRecord(opts._comments_after);
    /** @type {Record<string, string>} */
    const commentsBefore = Object.create(null);
    /** @type {Record<string, string>} */
    const commentsAfter = Object.create(null);
    Object.keys(rawBefore).forEach((key) => { const val = rawBefore[key]; if (typeof val === 'string') commentsBefore[key] = val; });
    Object.keys(rawAfter).forEach((key) => { const val = rawAfter[key]; if (typeof val === 'string') commentsAfter[key] = val; });
