...
- 2026-02-18: `base.py` now canonicalizes VAE memory-manager target (`vae.patcher` when present, else wrapper) across first-stage encode/decode and engine-level unload cleanup, preventing wrapper-vs-patcher record splits that caused redundant VAE unload/reload churn inside hires decode paths.
