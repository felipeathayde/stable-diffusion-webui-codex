...
- 2026-02-18: `base.py` now canonicalizes patcher-backed memory-manager targets (`component.patcher` when present, else component) across first-stage VAE encode/decode and engine-level unload cleanup (including `clipvision` and text-encoder fallback when `patcher is None`), preventing wrapper-vs-patcher record splits.
