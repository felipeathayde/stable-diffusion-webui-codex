...
- 2026-02-22: `routers/system.py` adds `POST /api/obliterate-vram` (aggressive VRAM cleanup for quick settings): runtime unload/cache purge + best-effort external compute-process termination via `nvidia-smi`, returning structured cleanup status for UI feedback.
