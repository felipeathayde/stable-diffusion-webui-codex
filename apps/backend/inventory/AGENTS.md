...
- 2025-12-04: Text encoder inventory continues to scan `models/text-encoder` but now also aggregates per-engine roots from `apps/paths.json` (`sd15_tenc`, `sdxl_tenc`, `flux1_tenc`, `wan22_tenc`, `zimage_tenc`), deduplicating by path so engine-specific TEnc folders show up once under `/api/models/inventory`.
