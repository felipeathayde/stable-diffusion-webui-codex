...
- `loras.py`: LoRA file discovery policy (models/Lora + per-family roots).
- `text_encoders.py`: Text encoder weight discovery policy (models/text-encoder + per-family roots).
- `wan22_gguf.py`: WAN22 stage GGUF discovery policy (paths.json `wan22_ckpt` or models/wan22) + stage classifier.
...
- Scanners intentionally ignore ad-hoc files under `models/` (only documented default subdirs + explicit roots from `apps/paths.json` are scanned).
