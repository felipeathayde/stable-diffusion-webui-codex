...
Defines per-family VAE roots via `apps/paths.json` keys (`*_vae`) and yields weight file paths in stable order.
...
def iter_vae_files(models_root: str | None = None) -> Iterable[str]:
    ...
    for key in ("sd15_vae", "sdxl_vae", "flux1_vae", "zimage_vae", "wan22_vae"):
