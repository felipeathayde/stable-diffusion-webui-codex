...
- 2026-03-06: `vae.py` decode hot paths now mirror ComfyUI again on output staging: decoded tensors are promoted to float32 on the output side, and decode tiled/non-tiled buffers accumulate in float32 before the final `[0,1]` clamp / `[-1,1]` handoff.
