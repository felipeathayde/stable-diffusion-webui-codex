...
- ControlNet patching routes now rely on `ControlNode`/`ControlGraph` (see `apps/backend/runtime/controlnet/`); use `UnetPatcher.add_control_node` instead of mutating linked lists.
