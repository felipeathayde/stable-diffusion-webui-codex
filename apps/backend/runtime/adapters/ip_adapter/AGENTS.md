...
- If isolated encoder/projector/resampler/attn2 proofs all match the official reference and the visual bug remains, debug the live integration seam with `CODEX_ZIMAGE_DEBUG_IP_ADAPTER_PATCH=1`: `session.py` owns the slot-map preview and `modules.py` owns the first live patch-call receipts (block, slot, sigma window, cond/uncond geometry, and tensor stats).
