...
- `loader.py` requires canonical core-transformer keyspace (no runtime key renaming), while `text_encoder.py` uses strict generic Qwen key-style normalization (`runtime/state_dict/keymap_qwen_text_encoder.py`) that only accepts known schemas/aux heads and fails loud on unknown keys.
