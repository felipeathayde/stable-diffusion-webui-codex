...
- `apps/backend/runtime/families/ltx2/model.py` — Typed bundle-planning dataclasses for components, external text encoder, and vendored metadata.
- `apps/backend/runtime/families/ltx2/text_encoder.py` — Strict one-asset Gemma3 resolution plus model/tokenizer loading from the vendored LTX2 metadata repo.
...
- `apps/backend/runtime/families/ltx2/audio.py` — Audio-VAE/vocoder bundle validation plus generated-audio WAV materialization into the shared `AudioExportAsset` contract.
...
- `runtime.py` must assemble through the local native package only: `load_ltx2_connectors(...)`, `load_ltx2_vocoder(...)`, and `load_strict_state_dict(...)` on transformer/video/audio VAEs. `load_ltx2_connectors(...)` owns optional `connectors.*` wrapper handling and must fail loud on mixed direct/wrapped surfaces. Do not reintroduce a generic `from_config(...)+load_state_dict(...)` path.
