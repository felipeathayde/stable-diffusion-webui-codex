...
- 2026-02-21: `sampling.make_scheduler(...)` no longer treats non-UniPC sampler strings as legacy no-op; WAN22 now fails loud on unsupported sampler/scheduler overrides (supports only `uni-pc[ bh1|bh2]` semantics with metadata-driven scheduler and `scheduler='simple'|default`).
