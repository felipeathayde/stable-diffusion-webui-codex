...
- 2026-02-12: `text_context.py` GGUF TE loading no longer hard-codes `dequantize=True`; it now builds/loads under `using_codex_operations(..., weight_format="gguf")`, delegates dequant policy to `runtime.checkpoint.io.load_gguf_state_dict` (`--gguf-exec` contract), constructs UMT5 with `transformers.modeling_utils.no_init_weights()` to avoid init crashes with GGUF op shims, avoids whole-model `enc.to(cuda)` residency for GGUF TE path, explicitly places non-quant TE params/buffers on execution device+dtype to prevent mixed-device LayerNorm failures, and re-aligns `CodexParameter.computation_dtype` to requested runtime dtype (CPU `fp32` contract).
