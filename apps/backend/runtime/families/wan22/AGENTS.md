...
- 2026-02-21: `stream_img2vid_chunked(...)` runs in chunk-major order (per chunk: high stage -> low stage in latent space, then final decode/stitch pass) so chunk `N+1` continuity anchors consume the low-final tail latent from chunk `N` (not high-stage partial latents).
