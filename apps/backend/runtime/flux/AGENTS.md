...
- `FluxTransformer2DModel.forward` now accepts extra keyword-only arguments (`control`, `transformer_options`, plus ignored `**kwargs`) so it can be driven safely via `KModel.apply_model` without relying on them; do not use these for new behaviour without coordinating with `apps/backend/runtime/modules/k_model.py`.
