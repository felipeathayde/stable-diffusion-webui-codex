...
- 2026-02-25: Kernel enforces streaming invariants at runtime (fail-loud): computes tile/workspace budgets, auto-resolves deterministic power-of-two `(q_chunk, kv_chunk)` pairs under caps, and still rejects long-sequence full-attention tile collapse (`q_chunk==q_len` and `kv_chunk==kv_len`) beyond bypass limits.
...
- For fail-loud cache/streaming guard errors, first inspect computed budget numbers in the invariant error. Remaining hard failures should be limited to cases where K/V cache residency already exhausts workspace cap (or no feasible budgeted pair exists).
- If you need stricter residency, lower `CODEX_WAN_FUSED_V1_Q_CHUNK` / `CODEX_WAN_FUSED_V1_KV_CHUNK`; kernel will still auto-resolve within those upper bounds.
