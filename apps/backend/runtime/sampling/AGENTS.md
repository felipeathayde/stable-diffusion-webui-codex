...
- 2026-03-07: `driver.py` now uses the RF/CONST-specific ancestral update whenever the active flow predictor reports `prediction_type='const'`; non-const predictors keep the generic ancestral Euler path, stochastic sampler step draws now use the shared deterministic `ImageRNG` / noise-settings policy for both `euler a` and native `er sde` instead of ambient `torch.randn_like(...)`, native `er sde` now offsets the first `prediction_type='const'` sigma before half-logSNR conversion so the first step does not hit the `sigma==1` singular boundary, `SamplerKind.UNI_PC_BH2` now runs a dedicated BH2 predictor/corrector coefficient path with multistep denoised-history handling (instead of aliasing plain `uni-pc`), and `sigma_schedules.py` now fails loud when `simple_schedule_mode='tail_downsample_sigmas'` is asked for more steps than the base `predictor.sigmas` ladder length.
- 2026-03-08: `driver.py` now treats flow/CONST `SIMPLE` double-zero tails (`..., 0, 0`) as a supported terminal no-op for `SamplerKind.UNI_PC_BH2` instead of raising on the preterminal zero; fail-loud guards remain for non-finite, negative, and non-monotonic zero→positive sigma transitions. The BH2 lane remains a parity target path, not a blanket exact-parity guarantee against every external implementation.
- 2026-03-08: `sigma_schedules.py` `linear_quadratic` now uses corrected piecewise coefficient construction plus strict input guards (`threshold_noise` finite, `1 <= linear_steps < steps`) to align schedule shape with the parity-target behavior.
- 2026-03-08: `driver.py` now provides native `SamplerKind.DPM_FAST` and `SamplerKind.DPM_ADAPTIVE` execution paths (Comfy/k-diffusion DPM-Solver parity target) with wrapper-aligned sigma bound selection (`sigma_min` from the last strictly-positive active sigma entry, including multi-zero tails such as SIMPLE/CONST `[..., 0, 0]`), strict fail-loud checks on sigma/time/PID validity, and current-contract defaults for missing adaptive/eta options (`eta=0`, `s_noise=1`, adaptive order/tolerance defaults).
- 2026-03-08: `catalog.py` / `registry.py` now treat executable support as implementation-backed authority; unsupported sampler rows remain catalog inventory with `supported=false`, the registry excludes them from active resolution, and `/api/samplers` callers must not infer executability from enum/catalog presence alone.
