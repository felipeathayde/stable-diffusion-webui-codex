...
def _maybe_dump_preview_factors(processing: Any, denoised_latent: torch.Tensor, *, step: int, total: int) -> None:
    global _DEBUG_PREVIEW_FACTORS_LAST_JOB_TS

    if not _truthy(os.getenv("CODEX_DEBUG_PREVIEW_FACTORS")):
        return
    ...
    latent = denoised_latent.detach().float()[0].movedim(0, -1).reshape(-1, int(denoised_latent.shape[1]))
    ...
    max_samples_raw = os.getenv("CODEX_DEBUG_PREVIEW_FACTORS_SAMPLES", "4096")
    try:
        max_samples = max(256, int(max_samples_raw))
    except Exception:
        max_samples = 4096
    sample_n = min(max_samples, n_pixels)
    ...
    factors = sol[:-1].detach().cpu().tolist()
    bias = sol[-1].detach().cpu().tolist()
    ...
    logger.info(
        ...
        type(processing.sd_model).__name__,
        int(denoised_latent.shape[1]),
        int(step),
        int(total),
        scale_h,
        scale_w,
    ...
    logger.info("[preview-factors] factors = %s", tuple(tuple(round(float(v), 6) for v in row) for row in factors))
    logger.info("[preview-factors] bias = %s", tuple(round(float(v), 6) for v in bias))
...
def execute_sampling(
    ...
    *,
    ...
    def _preview_cb(denoised_latent: torch.Tensor, step: int, total: int) -> None:
        debug_factors = _truthy(os.getenv("CODEX_DEBUG_PREVIEW_FACTORS"))
        ...
        try:
            if total is not None and int(total) > 0 and int(step) >= int(total):
                if debug_factors:
                    _maybe_dump_preview_factors(processing, denoised_latent, step=int(step), total=int(total))
                return
        except Exception:
        ...
        if debug_factors:
            _maybe_dump_preview_factors(processing, denoised_latent, step=int(step), total=int(total or 0))
