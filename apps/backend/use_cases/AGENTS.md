...
- 2026-02-21: `vid2vid.py` flow-chunks now sanitizes inner `Img2VidRequest.extras` by removing `img2vid_chunk_*` controls to prevent nested chunk recursion inside each flow chunk call (avoids repeated chunked reruns and extra VAE encode pressure).
