...
- 2026-02-18: `run_txt2img` and `run_img2img` now execute sampling + decode + post-cleanup inside the same worker-thread envelope; smart runtime overrides stay active for the full image-job lifecycle (no decode/cleanup main-thread split path).
