...
- 2026-02-20: `_log_conditioning(...)` now emits cond/uncond diagnostics via `emit_backend_event(...)` (`sdxl.conditioning.cond` / `sdxl.conditioning.uncond`) with normalized shape tokens (`BxSxC`) so high-field SDXL logs use the same structured multiline formatting as other backend telemetry.
