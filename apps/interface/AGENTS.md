...
- 2025-11-14: API requests are built via `src/api/payloads.ts` (Zod schemas) — payload builders trim prompts but now allow empty strings and always attach the per-tab engine/model metadata (even for img2img).
