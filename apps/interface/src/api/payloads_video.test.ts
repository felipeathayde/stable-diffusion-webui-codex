...
describe('WAN video payload builders', () => {
  ...
  it('omits interpolation payload when target FPS is not above base FPS', () => {
    ...
    const payload = buildWanTxt2VidPayload({
      ...
      format: 'auto',
      assets: {
        metadataRepo: 'Wan-AI/Wan2.2-T2V-A14B-Diffusers',
        textEncoderSha: sha,
        vaeSha: sha,
      },
      output: {
  ...
  it('rounds interpolation factor up when target FPS is not divisible by base FPS', () => {
    const sha = 'f'.repeat(64)
    const payload = buildWanTxt2VidPayload({
      device: 'cpu',
      settingsRevision: 9,
  ...
  it('builds an img2vid sliding payload with window controls', () => {
    ...
    expect(payload.img2vid_window_commit_frames).toBe(16)
  ...
  it('builds an img2vid svi2 payload with window controls', () => {
  ...
  it('normalizes window stride/commit for windowed img2vid modes', () => {
    ...
    const payload = buildWanImg2VidPayload({
      ...
      attentionMode: 'sliding',
      initImageData: 'data:image/png;base64,BBBB',
      img2vidMode: 'svi2_pro',
      windowFrames: 25,
      windowStride: 11,
      windowCommitFrames: 10,
      anchorAlpha: 0.5,
      chunkSeedMode: 'increment',
      high: {
        modelSha: sha,
    ...
    expect(payload.img2vid_window_stride).toBe(8)
    expect(payload.img2vid_window_commit_frames).toBe(12)
  })
