...
- 2026-03-02: `RefinerSettingsCard.vue` now uses a single slider row for swap-model controls (`Swap At Step` + `CFG`), removes the refiner-seed input from UI, and adds a CFG-side `Advanced` toggle that expands APG sliders (`APG Start/Eta/Rescale/Momentum/Norm`) gated by `guidanceSupport`.
