...
- 2026-03-01: `InitialImageCard.vue` and `Img2ImgInpaintParamsCard.vue` now expose optional `zoomFrameGuide` pass-through plus `update:zoomFrameGuide` forwarding from `ImageZoomOverlay.vue`, so WAN init-image zoom can edit no-stretch framing (`resizeMode`, guide `W/H`, and drag-based crop offsets) with parent-owned state.
