...
- 2026-01-13: Checkpoint metadata payload uses `file.name`, `file.path`, and `file.size.*` keys (no `title`/`model_name`/`filename`) and the file-metadata keys are normalized (e.g. `model.*`, `codex.*`, `gguf.*`).
