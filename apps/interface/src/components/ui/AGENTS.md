...
- 2026-03-24: `InpaintMaskEditorOverlay.vue` now reuses parent-owned `maskBlur` / `maskedPadding` state via shared sliders, renders the same outward blur-spill band + cyan effective-crop box used by the inline thumbnail, and falls back to `imageWidth` / `imageHeight` when preview-only processing dimensions are not provided. `maskInvert` now affects the display-only effective mask shown in the editor (red mask + crop/blur preview) while apply/export keep the raw draft mask unchanged.
