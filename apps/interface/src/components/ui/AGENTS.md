...
- 2026-03-01: `ImageZoomOverlay.vue` now supports optional WAN frame-guide editing for init-image previews: guide toggle, no-stretch resize-mode select (`auto|fit_width|fit_height`), guide `W/H` controls, drag-based crop offsets, and source/scaled/frame/crop metadata bound to deterministic projection math.
