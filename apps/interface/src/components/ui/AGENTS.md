...
- 2026-03-24: `InpaintMaskEditorOverlay.vue` now reuses parent-owned `maskBlur` / `maskedPadding` state via shared sliders, renders the same outward blur-spill band + cyan effective-crop box used by the inline thumbnail, and falls back to `imageWidth` / `imageHeight` when preview-only processing dimensions are not provided.
