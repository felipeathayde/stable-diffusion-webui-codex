...
- 2026-03-24: `InpaintMaskEditorOverlay.vue` now reuses parent-owned `maskBlur` / `maskedPadding` state via shared sliders, renders an orange blur-halo preview plus cyan effective-crop box from the current working mask, and falls back to `imageWidth` / `imageHeight` when preview-only processing dimensions are not provided.
