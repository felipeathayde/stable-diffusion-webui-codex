...
- 2025-12-06: `flux.ts` injeta `textEncoderOverride` nos payloads de `/flux` usando os dois selects de text encoder da QuickSettings (labels `flux/<abs_path>`) e o helper `deriveFluxTextEncoderOverrideFromLabels`; model tabs usam o mesmo helper via `useGeneration` quando `tab.type === 'flux'` e `tab.params.textEncoders` está preenchido.
