...
- 2026-03-02: `styles/components/views-shared.css` now adds a small top gap to `.panel-stack--sticky` (`top: calc(--sticky-offset + space)`) and pins the first panel inside that stack (`.panel-stack--sticky > .panel:first-child`) so RUN remains sticky even when Results grows taller than the left column.
