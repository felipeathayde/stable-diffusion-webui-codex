...
- 2026-03-25: `styles.css` now keeps `.init-dropzone-actions` as an in-flow row inside `.init-dropzone-slot` instead of an absolute top-right overlay, so init-image actions stay above the preview surface and no longer cover image-space overlays.
