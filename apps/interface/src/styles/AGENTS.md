...
- 2025-12-22: `components/wan22-settings.css` now hosts shared “number-with-controls” styles for WAN grids (steps/CFG/seed) and switches `.wan22-grid` from CSS grid to flex-wrap; `components/generation-settings-card.css` defines `w-cfg`, centers the stepper overlay, and adds a flex footer (`.gc-footer`) so CFG can sit next to Seed.
