...
- 2025-12-17: `WANTab.vue` moves Mode selection into QuickSettings, adds “Guided gen” (pulse + tooltip focus for missing prerequisites), and keeps a top-of-column sticky Results action bar (Generate + Snapshot). `WorkflowsList.vue` now uses a shared store and correctly renders workflow fields (`source_tab_id`, `created_at`).
