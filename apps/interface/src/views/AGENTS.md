...
- 2026-03-02: `ImageModelTab.vue` RUN header now shows an `XYZ` badge while XYZ workflow is enabled, and Run generate label is always `Generate` (no `Generate xyz` variant); XYZ stop/cancel stays centralized in RUN card actions.
