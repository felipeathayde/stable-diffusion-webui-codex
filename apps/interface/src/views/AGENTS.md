...
- 2025-12-22: `WANTab.vue` now uses sliders for core video params (width/height/frames/fps) and stage params (steps/CFG), seed actions (🎲/↺) are embedded inside the seed input, and `.wan22-grid` uses flex-wrap (cards don’t rely on hard grid columns).
