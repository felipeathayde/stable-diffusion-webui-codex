...
Purpose: Canonical baseline video workspace view.
This tranche installs `VideoModelTab.vue` as the baseline owner for the video-generation body by mechanically copying the current strongest live video workspace into `views/`,
without simplifying its behavior in the same change.
It owns the current prompt/init-media flow, stage/video/assets params, guided-generation overlay, history, task submission, progress/results, and export tooling surface,
while preserving the live implementation details already wired inside this body.

Symbols (top-level; keep in sync; no ghosts):
- `VideoModelTab` (component): Canonical baseline video workspace view.
...
- `defaultStage` (function): Returns default stage params (high/low) for new tabs/resets.
...
- `normalizeFrameCount` (function): Clamps/snap-normalizes frame counts to the current `4n+1` domain.
...
- `normalizeVideoPatch` (function): Sanitizes video patch updates before persisting tab params.
...
- `wanStageSamplers` / `wanStageSchedulers` (computed): Current sampler/scheduler inventories used by the stage selectors (`uni-pc*` + `euler` lanes, `simple` scheduler only).
- `wanRecommendedSamplers` / `wanRecommendedSchedulers` (computed): Sanitized recommendation lists forwarded into the current stage selectors when available.
...
- `onZoomFrameGuideUpdate` (function): Applies zoom-overlay frame-guide edits back into the video params.
- `clearInit` (function): Clears init image fields.
- `wanInitImageZoomFrameGuide` (computed): Derived init-image zoom frame-guide config from current video dimensions + image-scale/crop state.
