...
- 2026-02-23: `profiles.py` now performs persistent legacy allocator key migration (`PYTORCH_CUDA_ALLOC_CONF` -> `PYTORCH_ALLOC_CONF`, `CODEX_ENABLE_DEFAULT_PYTORCH_CUDA_ALLOC_CONF` -> `CODEX_ENABLE_DEFAULT_PYTORCH_ALLOC_CONF`) before pruning stale `*_ALLOC_CONF` keys, so normalized keys are written back to `.sangoi/launcher/*`.
- 2026-02-23: `services.py` allocator contract sanitization now migrates legacy allocator keys into canonical keys before spawn, then drops legacy/unsupported variants.
