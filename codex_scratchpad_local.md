...
- Durable `.sangoi` patrol taxonomy: scout-family patrol reports use `High | Medium | Low | Hypothesis` with `Inference flag: Fact | Inference` and required `Missing evidence` for `Inference`; reviewer patrol reports use `Blocker | High | Medium | Low | Hypothesis` with concrete evidence for `Blocker`/`High` and required missing-evidence callouts for `Hypothesis`. Do not collapse patrol docs/tooling into priority-bucket shorthand.
