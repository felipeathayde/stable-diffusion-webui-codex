...
def encode_to_base64(image):
    if isinstance(image, str):
        ...
        return codex_media.encode_pil_to_base64(image)
...
def encode_np_to_base64(image):
    ...
    return codex_media.encode_pil_to_base64(pil)
...
def controlnet_api(_: gr.Blocks, app: FastAPI):
    ...
    async def detect(
        ...
        for input_image in controlnet_input_images:
            img = np.array(codex_media.decode_base64_to_image(input_image)).astype('uint8')
