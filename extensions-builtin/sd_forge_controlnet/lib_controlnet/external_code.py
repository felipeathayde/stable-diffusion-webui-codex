...
from apps.backend.interfaces.api import media_helpers as codex_media
...
class ControlNetUnit:
    ...
    def from_dict(d: Dict) -> "ControlNetUnit":
        ...
        if isinstance(unit.image, str):
            unit.image = np.array(codex_media.decode_base64_to_image(unit.image)).astype('uint8')
        if isinstance(unit.mask_image, str):
            unit.mask_image = np.array(codex_media.decode_base64_to_image(unit.mask_image)).astype('uint8')
        return unit
...
def to_base64_nparray(encoding: str):
    ...
    return np.array(codex_media.decode_base64_to_image(encoding)).astype('uint8')
