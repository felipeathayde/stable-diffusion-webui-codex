...
def calculate_weight_fooocus(weight, alpha, v):
    w1 = memory_management.cast_to_device(v[0], weight.device, torch.float32)
    if w1.shape == weight.shape:
        w_min = memory_management.cast_to_device(v[1], weight.device, torch.float32)
        w_max = memory_management.cast_to_device(v[2], weight.device, torch.float32)
        w1 = (w1 / 255.0) * (w_max - w_min) + w_min
        weight += alpha * memory_management.cast_to_device(w1, weight.device, weight.dtype)
    else:
...
class FooocusInpaintPatcher(ControlModelPatcher):
    ...
    def __init__(self, state_dict, filename):
        ...
        self.inpaint_head.load_state_dict(utils.load_torch_file(os.path.join(os.path.dirname(__file__), 'fooocus_inpaint_head')))
    ...
    backend_lora.extra_weight_calculators['fooocus'] = calculate_weight_fooocus
