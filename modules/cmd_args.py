...
from apps.server.backend.config.args import parser
...
def _normalized_unquoted_filepath(p: str):
    try:
        # Strip surrounding quotes if present (Windows users often set env with quotes)
        if isinstance(p, str) and len(p) >= 2 and ((p[0] == p[-1] == '"') or (p[0] == p[-1] == "'")):
            p = p[1:-1]
    except Exception:
        pass
    return normalized_filepath(p)

parser.add_argument(
    "--ckpt-extra-dir",
    type=_normalized_unquoted_filepath,
    action='append',
    default=None,
    help="Additional directory with checkpoints. Can be passed multiple times.")
