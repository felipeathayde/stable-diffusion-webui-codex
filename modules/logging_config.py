...
def _build_handler(level: int) -> logging.Handler:
    # Allow forcing plain logs (no Rich) via env var
    no_rich_env = os.environ.get("SD_WEBUI_NO_RICH", "").strip().lower()
    force_plain = no_rich_env in {"1", "true", "yes", "on"}
    if not force_plain and RichHandler is not None and Console is not None:
