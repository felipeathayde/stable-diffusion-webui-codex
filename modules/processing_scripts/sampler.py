...
class ScriptSampler(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        ...
        if shared.opts.samplers_in_dropdown:
            # Render our HTML dropdown host instead of Gradio Dropdown for full UI control.
            # The frontend `html-dropdown.mjs` will mount the control based on this payload.
            import html as _html
            import json as _json

            def _payload(elem_id: str, label: str, choices: list[str], value: str) -> str:
                data = {
                    "id": elem_id,
                    "label": label,
            ...
            with FormRow(elem_id=f"sampler_selection_{self.tabname}"):
                self.sampler_name = gr.HTML(
                    value=_payload(f"{self.tabname}_sampling", 'Sampling method', sampler_names, default_sampler),
                    elem_id=f"{self.tabname}_sampling",
                )
                self.scheduler = gr.HTML(
                    value=_payload(f"{self.tabname}_scheduler", 'Schedule type', scheduler_names, default_scheduler),
        ...
        try:
            ...
            def _update(engine: str):
                ...
                # Rebuild HTML payloads for both hosts; HTML component accepts only `value` content updates.
                import html as _html2, json as _json2
                def _html_payload(elem_id: str, label: str, ch: list[str], val: str) -> str:
                    data = {
                        "id": elem_id,
                        "label": label,
                        "choices": [{"value": c, "label": c} for c in ch],
                        "value": val,
                        "allow_custom": False,
                        "multiselect": False,
                        "interactive": True,
                ...
                return (
                    gr.update(value=_html_payload(f"{self.tabname}_sampling", 'Sampling method', sn, sn[0] if sn else '')),
                    gr.update(value=_html_payload(f"{self.tabname}_scheduler", 'Schedule type', sc, sc[0] if sc else '')),
                )
