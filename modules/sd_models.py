...
class CheckpointInfo:
    def __init__(self, filename):
        ...
        # Consider primary ckpt_dir and any extra dirs for relative naming
        extra_dirs = []
        try:
            extra_raw = getattr(shared.cmd_opts, 'ckpt_extra_dir', None)
            if isinstance(extra_raw, (list, tuple)):
                extra_dirs = [os.path.abspath(x) for x in extra_raw if x]
            elif isinstance(extra_raw, str) and extra_raw:
                extra_dirs = [os.path.abspath(extra_raw)]
        except Exception:
            extra_dirs = []
        primary_dir = os.path.abspath(shared.cmd_opts.ckpt_dir) if shared.cmd_opts.ckpt_dir is not None else None
        candidate_dirs = [d for d in ([primary_dir] + extra_dirs + [model_path]) if d]

        self.is_safetensors = os.path.splitext(filename)[1].lower() == ".safetensors"

        name = os.path.basename(filename)
        # Prefer longest matching prefix for a clean relative name
        for base in sorted(candidate_dirs, key=lambda p: len(p or ''), reverse=True):
            try:
                base_abs = os.path.abspath(base)
                if abspath.startswith(base_abs):
                    name = abspath.replace(base_abs, '')
                    break
            except Exception:
                continue
...
def list_models():
    ...
    # Collect models from default path, primary --ckpt-dir and any --ckpt-extra-dir entries
    exts = [".ckpt", ".safetensors", ".gguf"]
    blacklist = [".vae.ckpt", ".vae.safetensors"]

    seen = set()
    model_list = []

    def _accumulate_from(cmd_path):
        paths = modelloader.load_models(
            model_path=model_path,
            model_url=None,
            command_path=cmd_path,
            ext_filter=exts,
            download_name=None,
            ext_blacklist=blacklist,
        )
        for p in paths:
            if p not in seen:
                seen.add(p)
    ...
    def _report(path, before_count):
        scanned = path if isinstance(path, str) else '<default>'
        delta = len(seen)
        count = delta - before_count
        scan_report.append((scanned, count))

    before = len(seen)
    _accumulate_from(shared.cmd_opts.ckpt_dir)
    _report(shared.cmd_opts.ckpt_dir, before)
    # extras may be list/tuple or None
    extra_dirs = getattr(shared.cmd_opts, 'ckpt_extra_dir', None) or []
    if isinstance(extra_dirs, (list, tuple)):
        for extra in extra_dirs:
            before = len(seen)
            _accumulate_from(extra)
            _report(extra, before)
    elif isinstance(extra_dirs, str) and extra_dirs:
        before = len(seen)
        _accumulate_from(extra_dirs)
        _report(extra_dirs, before)

    try:
        total = len(model_list)
        bases = [model_path]
        if shared.cmd_opts.ckpt_dir:
            bases.append(shared.cmd_opts.ckpt_dir)
        if isinstance(extra_dirs, (list, tuple)):
            bases.extend(list(extra_dirs))
        elif isinstance(extra_dirs, str) and extra_dirs:
            bases.append(extra_dirs)
        print(f"[Models] Scanned checkpoint dirs: {bases}; found {total} models.")
        for base, cnt in scan_report:
            print(f"[Models]  - {base}: +{cnt}")
    except Exception:
        pass
...
def get_checkpoint_state_dict(checkpoint_info: CheckpointInfo, timer):
    ...
    res = backend_utils.load_torch_file(checkpoint_info.filename)
...
def apply_token_merging(sd_model, token_merging_ratio):
    ...
    from apps.server.backend.runtime.misc.tomesd import TomePatcher
...
def forge_model_reload():
    ...
    try:
        from apps.server.backend.runtime.trace import event, trace_section
    except Exception:
        # tracing is optional
        class _Null:
            def __enter__(self):
