...
ui_codex_engine: gr.Component | None = None
ui_codex_mode: gr.Component | None = None

ui_checkpoint: gr.Component | None = None
ui_vae: gr.Component | None = None
ui_text_encoders: gr.Component | None = None
...
def _dropdown_model_selector(*, label: str, value, choices, elem_id: str | None = None, multiselect: bool = False):
    """Render HTML host for model selection (HTML-only dropdown).

    We emit a payload consumed by the frontend helper to build a native control.
    """
    import html as _html
    import json as _json
    elem = elem_id or f"model_select_{label.lower().replace(' ', '_')}"
    # Resolve callable value
    v = value() if callable(value) else value
    data = {
        "id": elem,
        "label": label,
        "choices": [{"value": str(c), "label": str(c)} for c in (choices() if callable(choices) else choices)],
        "value": v if (not multiselect) else (v if isinstance(v, list) else []),
        "allow_custom": False,
        "multiselect": bool(multiselect),
        "interactive": True,
    }
    payload = _html.escape(_json.dumps(data), quote=True)
    html_value = f'<div class="sdw-dropdown-host" data-sdw-dropdown="{payload}"></div>'
    return gr.HTML(value=html_value, elem_id=elem, elem_classes=['model_selection'], container=True)
...
def make_checkpoint_manager_ui():
    ...
    ui_codex_engine = _dropdown_model_selector(label="Engine", value=_engine_default_value, choices=engine_choices, elem_id="codex_engine", multiselect=False)
    ...
    ui_codex_mode = _dropdown_model_selector(label="Mode", value=_mode_default_value, choices=["Normal", "LCM", "Turbo", "Lightning"], elem_id="codex_mode", multiselect=False)
    ...
    def gr_refresh():
        # Ensure we rescan disk for new checkpoints (includes --ckpt-extra-dir)
        try:
            sd_models.list_models()
        except Exception:
            pass
        ckpts, vaes, text_encs = refresh_models()
        # Rebuild HTML hosts with updated payloads
        import html as _html2, json as _json2
        def _host(elem_id: str, label: str, choices, value, multiselect=False):
            data = {
                "id": elem_id,
                "label": label,
                "choices": [{"value": str(c), "label": str(c)} for c in choices],
                "value": value,
                "allow_custom": False,
                "multiselect": bool(multiselect),
                "interactive": True,
            }
            return f'<div class="sdw-dropdown-host" data-sdw-dropdown="{_html2.escape(_json2.dumps(data), quote=True)}"></div>'
        return (
            gr.update(value=_host("sd_checkpoint", "Checkpoint", ckpts, getattr(shared.opts, 'sd_model_checkpoint', ""))),
            gr.update(value=_host("sd_vae", "VAE", _compose_vae_choices(), _current_vae_value())),
            gr.update(value=_host("sd_text_encoders", "Text Encoder(s)", text_encs, _current_text_encoder_values(), multiselect=True)),
        )
    ...
    # HTML hosts have no .change(); we keep backend functions available for future JS bridges.
    try:
